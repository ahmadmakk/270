    ...
    char* generate_medium_bot_move(MediumBot* MediumBot, Grid PalyerGrid) {
        ...
        else if (MediumBot->base.Artillery){
    ...
    // if last move was radar we check the 2X2 grid of which hit_coord is the top left corner
        ...
        for (int i = 0; i < 4; i++) {
        addto_hit_stack(MediumBot, candidates[i]);
        }
    ...
    void addto_hit_stack(MediumBot *MediumBot, int candidate[2]){
            int row = candidate[0];
            int col = candidate[1];
    ...
    int down_Up_left_right[4][2] = {{row - 1, col}, {row + 1, col}, {row, col - 1}, {row, col + 1}};
        ...
        addto_hit_stack(MediumBot, down_Up_left_right[0]);
        addto_hit_stack(MediumBot, down_Up_left_right[1]);
        addto_hit_stack(MediumBot, down_Up_left_right[2]);
        addto_hit_stack(MediumBot, down_Up_left_right[3]);
        }
        //vertical orientation
        if(MediumBot->orientation == 1){
            if (PlayerGrid.display[row+1][col] == '*')
            addto_hit_stack(MediumBot, down_Up_left_right[1]);
            else
            addto_hit_stack(MediumBot, down_Up_left_right[0]);
        }
        ...
        if(MediumBot->orientation == 2){
            if (PlayerGrid.display[row][col+1] == '*')
            addto_hit_stack(MediumBot, down_Up_left_right[2]);
            else
            addto_hit_stack(MediumBot, down_Up_left_right[3]);
    ...
    int down_Up_left_right[4][2] = {{row - 1, col}, {row + 1, col}, {row, col - 1}, {row, col + 1}};
        ...
        if (
        validateCoordinates(row+1, col) && PlayerGrid[row+1][col] == '*' 
        ||
         validateCoordinates(row+-1, col) && PlayerGrid[row+1][col] == '*' )
